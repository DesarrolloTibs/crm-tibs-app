    ...
    <div className="flex min-h-screen w-full overflow-hidden bg-white">
        ...
        <div className="w-full max-w-[400px] animate-in fade-in slide-in-from-left-8 duration-700">
          ...
          {success ? (
            <div className="text-left animate-in fade-in zoom-in duration-500">
              ...
              <button onClick={() => navigate('/login')} className="login-btn-premium py-5">ENTENDIDO</button>
            </div>
          ) : (
            <>
              <div className="mb-10 text-left">
                ...
                <h1 className="text-4xl font-black text-slate-900 tracking-tight">¿Olvidaste tu acceso?</h1>
                <p className="text-blue-600 text-[11px] font-black uppercase tracking-[0.3em] mt-2">Billy Sales & Services • Seguridad</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input type="email" placeholder="usuario@empresa.com" value={email} onChange={e => { setEmail(e.target.value); if (error) setError(''); }} className={`login-input-premium pl-12 ${error ? 'border-rose-300 bg-rose-50/30' : ''}`} required />
                  </div>
                </div>
                ...
                <button type="submit" className="login-btn-premium py-5" disabled={loading}>
                  {loading ? <div className="flex items-center gap-3"><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>ENVIANDO...</div> : 'ENVIAR ENLACE DE RECUPERACIÓN'}
                </button>
              </form>
            </>
          )}
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-50 items-center justify-center relative overflow-hidden border-l border-slate-100">
        ...
        <div className="relative z-10 text-center px-12 animate-in fade-in zoom-in-95 duration-1000">
          ...
          <h2 className="text-5xl font-black text-slate-900 leading-tight mb-4">Cuidamos de ti y de tu acceso.</h2>
          <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-md mx-auto">Inicia el proceso de recuperación y sigue construyendo el futuro con nosotros.</p>
        </div>
      </div>
    </div>
  );
