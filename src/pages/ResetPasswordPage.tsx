    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-50 p-8">
        <div className="max-w-md w-full glass-card p-12 text-center animate-in fade-in zoom-in duration-500">
          ...
          <Link to="/forgot-password" className="login-btn-premium py-4 block text-center">SOLICITAR NUEVO ENLACE</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen w-full overflow-hidden bg-white">
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white z-10 relative">
        <div className="w-full max-w-[400px] animate-in fade-in slide-in-from-left-8 duration-700">
          <Link to="/login" className="inline-flex items-center text-rose-600 hover:text-rose-700 mb-10 font-black text-[10px] uppercase tracking-widest transition-all hover:-translate-x-1 group">
            <ArrowLeft size={16} className="mr-2 transition-transform group-hover:-translate-x-1" /> Volver al inicio
          </Link>
          ...
          {success ? (
            <div className="text-left animate-in fade-in zoom-in duration-500">
              ...
              <p className="text-slate-500 font-medium leading-relaxed mb-8">Tu acceso ha sido restaurado correctamente. Ya puedes acceder a la plataforma con tus nuevas credenciales. ¡Qué bueno tenerte de vuelta!</p>
              <button onClick={() => navigate('/login')} className="login-btn-premium py-5">IR AL LOGIN</button>
            </div>
          ) : (
            <>
              <div className="mb-10 text-left">
                ...
                <h1 className="text-4xl font-black text-slate-900 tracking-tight">¡Vamos a restaurar tu acceso!</h1>
                <p className="text-rose-600 text-[11px] font-black uppercase tracking-[0.3em] mt-2">Billy Sales & Services • Seguridad</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 relative group">
                    ...
                    <div className="relative">
                      <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${error ? 'text-rose-400' : 'text-slate-400 group-focus-within:text-rose-500'}`} size={18} />
                      <input type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={confirmPassword} onChange={e => { setConfirmPassword(e.target.value); if (error) setError(''); }} className={`login-input-premium pl-12 pr-12 focus:border-rose-500 focus:ring-rose-500 ${error ? 'border-rose-300 bg-rose-50/30' : ''}`} required />
                    </div>
                  </div>
                </div>
                ...
                {error && (
                  <div className="bg-transparent border border-rose-200 text-rose-600 text-[10px] font-black uppercase tracking-widest text-center py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 animate-in slide-in-from-top-2">
                    <ShieldAlert size={14} className="stroke-[3]" /> {error}
                  </div>
                )}

                <button type="submit" className="login-btn-premium !bg-rose-600 hover:!bg-rose-700 py-5" disabled={loading}>
                  {loading ? <div className="flex items-center gap-3"><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>PROCESANDO...</div> : 'ESTABLECER NUEVA CONTRASEÑA'}
                </button>
              </form>
            </>
          )}
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-50 items-center justify-center relative overflow-hidden border-l border-slate-100">
        ...
        <div className="relative z-10 text-center px-12 animate-in fade-in zoom-in-95 duration-1000">
          ...
          <h2 className="text-5xl font-black text-slate-900 leading-tight mb-4">Cuidamos de ti y de tu información.</h2>
          <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-md mx-auto">Recupera tu entrada y sigue construyendo el futuro del equipo.</p>
        </div>
      </div>
    </div>
  );
