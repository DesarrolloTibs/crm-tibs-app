    ...
    <div className={`${showToolbar ? 'flex' : 'hidden'} md:flex flex-col sm:flex-row w-full md:w-auto gap-3`}>
      ...
      >
        <div className="flex-1 flex flex-col gap-1.5 max-h-[320px] overflow-y-auto pr-1">
          ...
          <Button variant="ghost" onClick={() => setArchivedFilter(archivedFilter==='archived'?'active':'archived')} className="!justify-between w-full !text-xs sm:!text-sm !text-gray-700 !px-2 !py-1 !rounded">
            <span>Oportunidades Archivadas</span>{archivedFilter==='archived' && <span className="text-indigo-600 font-extrabold text-sm">✓</span>}
          </Button>
          <Button variant="ghost" onClick={() => setArchivedFilter(archivedFilter==='all'?'active':'all')} className="!justify-between w-full !text-xs sm:!text-sm !text-gray-700 !px-2 !py-1 !rounded">
          ...
          <div className="flex items-center gap-0.5 px-2 py-1">
            {[1,2,3].map(star => (
              <Button key={star} variant="icon" onClick={() => setPriorityFilter(priorityFilter===star?null:star)} title={star===1?'Baja o mayor':star===2?'Media o mayor':'Alta'} className="!p-0.5 hover:!scale-110 !transform">
          ...
          {activeStages.map(stage => (
            <Button key={stage.id} variant="ghost" onClick={() => setStatusFilter(statusFilter===stage.id?'':stage.id)} className="!justify-start !gap-2 !text-xs !text-gray-700 !px-2 !py-1 !rounded w-full">
          ...
          <div className="border-t border-gray-100 my-1 shrink-0" />
          <Button variant="ghost" onClick={() => { setShowFilters(false); onOpenCustomFilter(); }} className="!gap-1 !text-xs !text-indigo-600 hover:!text-indigo-800 !px-2 !py-1.5 !rounded w-full !justify-start hover:!bg-indigo-50 !font-bold">+ Filtro personalizado...</Button>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-1.5 border-l border-gray-100 pl-4 max-h-[320px] overflow-y-auto">
          ...
          {contactsList.length > 0 ? (
            contactsList.map(c => (
              <Button key={c.id} variant="ghost" onClick={() => setContactFilter(contactFilter===c.id?'':c.id)} className="!justify-between !text-xs sm:!text-sm !text-gray-700 !px-2 !py-1 !rounded w-full">
        ...
        <div className="flex-1 flex flex-col gap-1.5 border-l border-gray-100 pl-4 max-h-[320px] overflow-y-auto">
          ...
          {executives.map(exec => (
            <Button key={exec.id} variant="ghost" onClick={() => setExecutiveFilter(executiveFilter===exec.id?'':exec.id)} className="!justify-between !text-xs sm:!text-sm !text-gray-700 !px-2 !py-1 !rounded w-full">
          ...
          <Button variant="ghost-danger" onClick={onClearFilters} className="gap-1.5 w-full !justify-start"><XCircle size={12} /> Limpiar Filtros</Button>
        </div>
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto">
        <div className="flex border border-gray-300 rounded-lg overflow-hidden p-0.5 bg-gray-50 shadow-sm shrink-0">
          <Button variant="ghost" onClick={() => setViewMode('kanban')} className={`!px-3 !py-1.5 !gap-1.5 !rounded-md !text-xs !font-semibold ${viewMode==='kanban'?'!bg-white !text-blue-600 shadow-sm':'!text-slate-500'}`} title="Vista Kanban">
            <KanbanIcon size={14} /><span>Kanban</span>
          </Button>
          <Button variant="ghost" onClick={() => setViewMode('list')} className={`!px-3 !py-1.5 !gap-1.5 !rounded-md !text-xs !font-semibold ${viewMode==='list'?'!bg-white !text-blue-600 shadow-sm':'!text-slate-500'}`} title="Vista Lista">
